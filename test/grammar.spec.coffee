...
describe "SQL Grammar", ->
	describe "SELECT Queries", ->
		...
		it "parses TOP clauses", ->
			parse("SELECT TOP 10 * FROM my_table").toString().should.eql """
			SELECT TOP 10 *
				FROM `my_table`
			"""

		it "parses SAMPLE clauses", ->
			parse("SELECT * FROM my_table SAMPLE 10").toString().should.eql """
			SELECT *
				FROM `my_table`
				SAMPLE 10
			"""
