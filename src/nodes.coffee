...
exports.Select = class Select
		constructor: (@top, @fields, @source, @distinct=false, @joins=[], @unions=[]) ->
			@order = null
			@group = null
			@where = null
			@limit = null
			@sample = null
		toString: ->
			# ret = ["SELECT#{indent(top.toString()) if @top} #{@fields.join(', ')}"]
			ret = if @top then ["SELECT #{@top.toString()} #{@fields.join(', ')}"] else ["SELECT #{@fields.join(', ')}"]
			...
			ret.push indent(@sample.toString()) if @sample
